...
* A trigram model, which is a a feedforward network that takes sequence elements $y_{t-2}$ and $y_{t-1}$ as input and generates element $y_t$ as output.

* A simple recurrent model, with a one-dimensional state, governed by the equations: $s_t = \tanh(W^{s,s} s_{t-1} + W^{s,x} y_{t-1})$ and $P(y_t = k \mid y_0, \ldots y_{t-1}) = {\rm softmax}(W^o s_t)$.
...
\begin{align} s_t &= w s_{t-1} + x_t \\ p_t &= s_t \end{align}
...
2A)
For an RNN that, given $x=[1,1,1]$ produces $p=[1,2,3]$, what is the value of $w$?
...
is the target output. What is $\frac{d\text{Loss}}{dw}$ where
$\text{Loss} = \sum_{t=1}^3\text{Loss}(p_t, y_t)$? Assume $s_0=0$.

Enter an expression in terms of (only) w, x_1, x_2, x_3,
y_1, y_2, y_3. Do not use any of the $p_t$.
...
Consider in addition a related model where instead of a single weight
$w$, at each time step we can use a different weight $w_t$; call this
the multi-w model.

Assume $s_0 = 0$ throughout.

3A)
Can the single-w model exactly represent a machine that takes $x=[1,1,1]$ and produces $y=[1,2,4]$? If so, what is the value of $w$? If not, enter 'None'.
...
3D)
Which of the following are reasons to prefer the multi-w model (over the single-w model)?
